...
class TestNumbaDotProductVectors(unittest.TestCase):
    def case_dot_product_vectors(self, vector1, vector2):
        np.testing.assert_equal(
            dot_product_vectors(vector1, vector2), np.dot(vector1, vector2)
        )
    ...
    def test_dot_product_vectors_large_values(self):
        self.case_dot_product_vectors(
            np.array([10**9, 10**9, 10**9]), np.array([1, 0, 0])
        )
        self.case_dot_product_vectors(
            np.array([-(10**9), -(10**9), -(10**9)]), np.array([1, 1, 1])
        )
